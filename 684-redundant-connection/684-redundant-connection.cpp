...
Time Complexity : O(n*α(n)) ≈ O(n), the time complexity of each find call after union-by-rank and path compression comes out to be O(α(n)), where α(n) is the inverse Ackermann function. It doesn't exceed 4 for any n < 10600 and hence is practically constant. We make O(n) calls in total.
