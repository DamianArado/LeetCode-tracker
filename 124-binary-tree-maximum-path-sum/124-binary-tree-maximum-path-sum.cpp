...
private:
    int calculate(TreeNode* node, int &maxPSum) {
        ...
        maxPSum = max(maxPSum, leftSum + rightSum + node->val); // store the max path sum in this
        return node->val + max(leftSum, rightSum); // to decide which path to take among the left and right as the longest path
    }
public:
