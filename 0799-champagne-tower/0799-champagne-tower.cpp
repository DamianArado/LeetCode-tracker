...
public:
    double champagneTower(int poured, int query_row, int query_glass) {
        ...
        for (int i = 0; i <= query_row; ++i) { //we need to make the dp matrix only till query row. No need to do after that
            vector<double> nextRow(i + 2); //If we are at row 0, row 1 will have 2 glasses. So next row will have currRow number + 2 number of glasses.
            ...
            if (i != query_row) 
                currRow = nextRow; //change the currRow for the next iteration. But if we have already reached the query_row, then the next iteration will not even take place, so the currRow is the query_row itself. So don't change as we need the currRow only.
        }
